<?php
class ImportWordFiltered extends Client {
	protected $parent = array();
	protected $word_options = array(
	...
	public function buildPagesData($file) {
		...
		$Word = new ImportWordHtml($this->Tools);
		$Word->convert($this->import, $this->word_options);
		$this->import = $Word->getPages();
		...
		foreach ($this->import as $page) {
			...
			$page_data = $this->buildPageData($page, $page_order, $Word->getScript(), $Word->getStyle());
	...
	protected function makeHiddenContentConditional($content) {
		...
		foreach ($content->find("span[style]") as $style) {
			...
			$style_attr = str_replace(array("\n", "\r"), '', $style_attr);

			// Skip when the item is not hidden.
			if (stripos($style_attr, 'display:none') === false) {
				continue;
			}
